...
?>
            ...
            <form method="post" name="contact" action="mail">
                <div class="col_13 float_l">
                    ...
                    <input type="checkbox" name="cv" id="cv" value="1" <?php if (file_exists($cv)) { echo 'checked="checked"'; } else { echo 'disabled="disabled"'; }; ?> />
                    <label class="checkbox" for="cv" <?php if (!file_exists($cv)) { ?> onclick="alert('Vous n\'avez pas de CV !\r\n Vous pouvez en enregistrer un avec l\'longlet Compte');" <?php } ?> >Joindre mon CV</label>   
                </div>
